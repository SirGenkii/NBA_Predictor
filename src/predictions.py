...
from src.config import FEATURES_TO_ROLL, DATA_ODDS_HISTORY_DIR
...
def build_prediction_rows(home_team_id: int, away_team_id: int, home_odds: float, away_odds: float ,dataset: pd.DataFrame, match_date: datetime = None) -> pd.DataFrame:
    ...
    temp_df = compute_rolling_features(temp_df, "TEAM_ID", ["TEAM_ID", "GAME_DATE"], FEATURES_TO_ROLL, N_LIST)

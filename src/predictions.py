...
def build_prediction_rows(home_team_id: int, away_team_id: int, home_odds: float, away_odds: float ,dataset: pd.DataFrame, match_date: datetime = None) -> pd.DataFrame:
    ...
    temp_df = compute_side_win_streak(temp_df, win_shifted_col="IS_WIN_SHIFTED")
    ...
    temp_df = compute_home_away_pts(temp_df, "TEAM_ID", "IS_HOME", "points_traditional", "OPP_points_traditional", N_LIST)
